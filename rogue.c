...
// 
 ...
 if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 4096) < 0) {
    ...
    Mix_Music *bgMusic = Mix_LoadMUS("/mnt/c/Users/amir8/Downloads/interstellar.Day-one(320).mp3");


    if (bgMusic == NULL) {
        printw("Failed to load beat music! SDL_mixer Error: %s\n", Mix_GetError());
        refresh();
        return -1;
    }
...
int roomsOverlap_1(Room *a, Room *b) {
    return (a->x <= b->x + b->width && a->x + a->width >= b->x &&
            a->y <= b->y + b->height && a->y + a->height >= b->y);
}

void drawRoom_1(Room *room, Map *map) {
...
void drawCorridor_1(Room *a, Room *b, Map *map) {
...
void initialize_enemy_1(Enemy *enemy, int x, int y, int room_index, char type) {
    enemy->x = x;
    enemy->y = y;
    enemy->symbol = type == 'S' ? 'S' : 'E';
    enemy->health = type == 'S' ? 100 : 50;
    enemy->damage = type == 'S' ? 50 : 10;
    enemy->speed = 1;
...
void initialize_fire_1(Fire *fire, int x, int y) {
    fire->x = x;
    fire->y = y;
    fire->symbol = '^';
    fire->damage = 10;
...
void initialize_bullet_1(Bullet *bullet, int x, int y, int dx, int dy) {
    bullet->x = x;
    bullet->y = y;
    bullet->symbol = '*';
    bullet->dx = dx;
...
void initialize_food_1(Food *food, int x, int y) {
    food->x = x;
    food->y = y;
...
void generate_random_map_1(Map *map) {
    srand(time(NULL));
    initialize_map_1(map);

    int roomCount = 0;

    while (roomCount < MAX_ROOMS) {
        Room newRoom;
        createRoom_1(&newRoom);

        int failed = 0;
        for (int i = 0; i < roomCount; i++) {
            if (roomsOverlap_1(&newRoom, &map->rooms[i])) {
                failed = 1;
                break;
            }
        }
        ...
        if (!failed || roomCount == 0) {
            drawRoom_1(&newRoom, map);

            if (roomCount != 0) {
                drawCorridor_1(&newRoom, &map->rooms[roomCount - 1], map);
            }
    ...
    for (int i = 0; i < MAX_ENEMIES; i++) {
        ...
        initialize_enemy_1(&map->enemies[i], 
...
void initialize_player_1(Player *player, int x, int y) {
    player->x = x;
    player->y = y;
    player->symbol = '@';
    player->health = 10;
...
void ensure_player_on_floor_1(Player *player, Map *map) {
...
void move_enemy_randomly_1(Enemy *enemy, Map *map) {
...
void move_toxic_enemy_1(Enemy *enemy, Player *player, Map *map) {
...
void move_boss_towards_player_1(Enemy *boss, Player *player) {
    if (rand() % 100 < 30) {
...
void fire_weapon_1(Player *player, Map *map) {
    if (player->ammo > 0) {
        player->ammo--;
        int damage_dealt = 0;
...
void create_boss_room_1(Map *map, Player *player) {
    initialize_map_1(map);
...
void activate_boss_1(Map *map, Player *player) {
    if (!map->boss_active) {
        map->boss_active = 1;
        create_boss_room_1(map, player);
        printw("Dragon Boss activated! Defeat it to win!\n");
    }
}

void move_player_1(Player *player, Map *map, int dx, int dy, int speed) {
    if (player->cheat_mode) {
        while (1) {
            ...
            print_map_with_player_1(map, player);
            refresh();
            napms(100); 
        }
    } else {
...
void print_map_with_player_1(Map *map, Player *player) {
    clear();
    int vision_radius = 2;
...
void game_menu_1(Map *map, Player *player) {
    initscr();
     music(1);
    ...
    while ((ch = getch()) != 'q') {
        ...
        for (int i = 0; i < map->enemy_count; i++) {
            if (map->enemies[i].type == 'B') {
                move_boss_towards_player_1(&map->enemies[i], player);
            } else if (map->enemies[i].type == 'S') {
                move_toxic_enemy_1(&map->enemies[i], player, map);
            } else {
                move_enemy_randomly_1(&map->enemies[i], map);
            }
        }
                    ...
                    for (int dx = -1; dx <= 4; dx++) {
                        for (int dy = -1; dy <= 4; dy++) {
                            ...
                            if (fx >= 0 && fx < MAP_WIDTH && 
                                fy >= 0 && fy < MAP_HEIGHT) {
                                initialize_fire_1(&map->fires[map->fire_count++], fx, fy);
                            }
                        }
                    }
...
int roomsOverlap_2(Room *a, Room *b) {
    return (a->x <= b->x + b->width && a->x + a->width >= b->x &&
            a->y <= b->y + b->height && a->y + a->height >= b->y);
}

void drawRoom_2(Room *room, Map *map) {
    for (int y = room->y; y < room->y + room->height; y++) {
        for (int x = room->x; x < room->x + room->width; x++) {
            if (y == room->y || y == room->y + room->height - 1 ||
                x == room->x || x == room->x + room->width - 1) {
...
void drawCorridor_2(Room *a, Room *b, Map *map) {
    int x1 = a->x + a->width / 2;
    int y1 = a->y + a->height / 2;
    int x2 = b->x + b->width / 2;
    int y2 = b->y + b->height / 2;
...
void initialize_enemy_2(Enemy *enemy, int x, int y, int room_index, char type) {
    enemy->x = x;
    enemy->y = y;
    enemy->symbol = type == 'S' ? 'S' : 'E';
    enemy->health = type == 'S' ? 100 : 50;
...
void initialize_fire_2(Fire *fire, int x, int y) {
    fire->x = x;
    fire->y = y;
    fire->symbol = '^';
    fire->damage = 12;
...
void initialize_bullet_2(Bullet *bullet, int x, int y, int dx, int dy) {
    bullet->x = x;
    bullet->y = y;
    bullet->symbol = '*';
    bullet->dx = dx;
...
void initialize_food_2(Food *food, int x, int y) {
    food->x = x;
    food->y = y;
    food->symbol = "opi"[rand() % 3]; 
    food->is_poisonous = rand() % 2;
...
void generate_random_map_2(Map *map) {
    srand(time(NULL));
    initialize_map_1(map);
    ...
    while (roomCount < MAX_ROOMS) {
        Room newRoom;
        createRoom_2(&newRoom);

        int failed = 0;
        for (int i = 0; i < roomCount; i++) {
            if (roomsOverlap_2(&newRoom, &map->rooms[i])) {
                failed = 1;
                break;
            }
        }
        ...
        if (!failed || roomCount == 0) {
            drawRoom_2(&newRoom, map);

            if (roomCount != 0) {
                drawCorridor_2(&newRoom, &map->rooms[roomCount - 1], map);
            }
    ...
    for (int i = 0; i < MAX_ENEMIES; i++) {
        ...
        if (rand() % 5 == 0) type = 'S'; 
        if (i == 0 && map->level % 3 == 0) type = 'B'; 
        initialize_enemy_2(&map->enemies[i], 
                         room->x + 1 + rand() % (room->width - 2), 
                         room->y + 1 + rand() % (room->height - 2),
                         roomIndex,
                         type);
...
void initialize_player_2(Player *player, int x, int y) {
    player->x = x;
    player->y = y;
    player->symbol = '@';
    player->health = 20;
...
void ensure_player_on_floor_2(Player *player, Map *map) {
    if (map->tiles[player->y][player->x] != '.') {
        int found = 0;
        for (int y = 0; y < MAP_HEIGHT && !found; y++) {
            for (int x = 0; x < MAP_WIDTH && !found; x++) {
...
void move_enemy_randomly_2(Enemy *enemy, Map *map) {
    Room *room = &map->rooms[enemy->room_index];
    int dx = rand() % 3 - 1;
    int dy = rand() % 3 - 1;
...
void move_toxic_enemy_2(Enemy *enemy, Player *player, Map *map) {
    int dx = (player->x > enemy->x) ? 1 : (player->x < enemy->x) ? -1 : 0;
    int dy = (player->y > enemy->y) ? 1 : (player->y < enemy->y) ? -1 : 0;
...
void move_boss_towards_player_2(Enemy *boss, Player *player) {
    if (rand() % 100 < 30) {
        if (boss->x < player->x) boss->x += boss->speed;
        else if (boss->x > player->x) boss->x -= boss->speed;
...
void fire_weapon_2(Player *player, Map *map) {
    if (player->ammo > 0) {
        player->ammo--;
        int damage_dealt = 0;
...
void create_boss_room_2(Map *map, Player *player) {
...
void activate_boss_2(Map *map, Player *player) {
    if (!map->boss_active) {
        map->boss_active = 1;
        create_boss_room_2(map, player);
        printw("Dragon Boss activated! Defeat it to win!\n");
    }
}

void move_player_2(Player *player, Map *map, int dx, int dy, int speed) {
    if (player->cheat_mode) {
        while (1) {
            int new_x = player->x + dx * speed;
            int new_y = player->y + dy * speed;
            ...
            print_map_with_player_2(map, player);
            refresh();
            napms(100); 
        }
    } else {
...
void game_menu_2(Map *map, Player *player) {
    initscr();
     music(1);
    ...
    while ((ch = getch()) != 'q') {
        ...
        for (int i = 0; i < map->enemy_count; i++) {
            if (map->enemies[i].type == 'B') {
                move_boss_towards_player_2(&map->enemies[i], player);
            } else if (map->enemies[i].type == 'S') {
                move_toxic_enemy_2(&map->enemies[i], player, map);
            } else {
                move_enemy_randomly_2(&map->enemies[i], map);
            }
        }
                ...
                if (rand() % 100 < 20) {
                    for (int dx = -1; dx <= 4; dx++) {
                        for (int dy = -1; dy <= 4; dy++) {
                            ...
                            if (fx >= 0 && fx < MAP_WIDTH && 
                                fy >= 0 && fy < MAP_HEIGHT) {
                                initialize_fire_2(&map->fires[map->fire_count++], fx, fy);
                            }
                        }
                    }
                    printw("BOSS breathes fire!\n");
...
int main() 
    ...
    while (1) {
        ...
        printf("2. Start game(easy mode)\n");
        printf("3. start game (medium mode))\n");
        printf("4. start game (difficult mode)\n");
        printf("5.exit game\n");
        ...
        if (choice == 1) {
            ...
            game_menu_2(&map, &player);
        } 
        else if(choice==4){
             game_menu_1(&map, &player);
        }
         else if (choice == 5) {
            break;
        } else {
