...
void game_menu(Map *map, Player *player) {
    initscr();
    start_color();
    init_pair(1, COLOR_RED, COLOR_BLACK);      // دشمن معمولی
    ...
    init_pair(6, COLOR_WHITE, COLOR_BLACK);    // بازیکن (پیشفرض)
    init_pair(7, COLOR_GREEN, COLOR_BLACK);    // دشمن سمی
    init_pair(8, COLOR_RED, COLOR_BLACK);      // غذای سمی
    init_pair(9, COLOR_BLUE, COLOR_BLACK);     // غذای سالم
    init_pair(10, COLOR_YELLOW, COLOR_BLACK);  // آیتم U
    cbreak();
    noecho();
    keypad(stdscr, TRUE);
    ...
    while ((ch = getch()) != 'q') {
        if (ch == 'v') {
            ...
            printw("Player color changed to %s!\n",
                  (player->current_color == 6) ? "WHITE" : "BLUE");
        } else {
            speed = 1;
        }
