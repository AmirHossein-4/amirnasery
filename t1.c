...
void initialize_enemy(Enemy *enemy, int x, int y, int room_index) {
    enemy->x = x;
    enemy->y = y;
    enemy->symbol = 'E';
    enemy->health = 50;
...
int check_room_password() {
    char input[20];
    printw("Enter exit password (1111): ");
    echo();
    scanw("%s", input);
    noecho();
        ...
        if (target_tile == '.' || (target_tile == '#' && player->ghost_mode)) {
            ...
            if (prev_room != -1 && new_room == -1) {
                for (int i = 0; i < MAX_EXIT_POINTS; i++) {
                    ...
                    if (new_x == ep.x && new_y == ep.y) {
                        if (!check_room_password()) {
                            return;
                        }
                        break;
                    }
...
void print_map_with_player(Map *map, Player *player) {
    ...
    for (int y = 0; y < MAP_HEIGHT; y++) {
        for (int x = 0; x < MAP_WIDTH; x++) {
            ...
            else if (dx <= vision_radius && dy <= vision_radius) {
                int printed = 0;

                // نمایش دشمنان
                for (int i = 0; i < map->enemy_count; i++) {
                    if (map->enemies[i].x == x && map->enemies[i].y == y) {
                        attron(COLOR_PAIR(map->enemies[i].is_boss ? 2 : 1));
                        printw("%c", map->enemies[i].symbol);
                        attroff(COLOR_PAIR(map->enemies[i].is_boss ? 2 : 1));
                        printed = 1;
                        break;
                    }
                }
    ...
    printw("Health: %d | Gold: %d | Score: %d | Level: %d | Ghost: %s | Ammo: %d\n", 
           player->health, player->gold, player->score, map->level,
           player->ghost_mode ? "ON" : "OFF", player->ammo);
    refresh();
}

void save_game(Map *map, Player *player) {
    ...
    if (file) {
        ...
        printw("Failed to save game.\n");
    }
    refresh();
}

void load_game(Map *map, Player *player) {
...
void game_menu(Map *map, Player *player) {
    initscr();
    start_color();
    init_pair(1, COLOR_RED, COLOR_BLACK);    // دشمنان معمولی
    init_pair(2, COLOR_MAGENTA, COLOR_BLACK); // باس
    init_pair(3, COLOR_YELLOW, COLOR_BLACK);  // آتش
    init_pair(4, COLOR_CYAN, COLOR_BLACK);    // آیتم‌ها
    init_pair(5, COLOR_GREEN, COLOR_BLACK);   // گلوله‌ها
    init_pair(6, COLOR_WHITE, COLOR_BLACK);   // بازیکن
    cbreak();
    noecho();
    keypad(stdscr, TRUE);
    ...
    while ((ch = getch()) != 'q') {
        ...
        switch (ch) {
            ...
            case 's': save_game(map, player); break;
            case 'l': load_game(map, player); break;
        }
        ...
        if (map->boss_active && map->enemy_count == 0) {
